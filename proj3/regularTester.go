...
func test(p, b, fn string) (r string, fail int) {
    f, _ := os.Open(fn)
            ...
            case "Exec":
                if inBlock == 0 {
                    ...
                    go func(){
                      ...
                      if err == nil {
                          r += fmt.Sprintf("Exec Success.\n")
                      } else {
                          r += fmt.Sprintf("Exec Error: %v\n", err)
                      }
                      flag = 0
                    }()
...
func main() {
    ...
    conf, _ := os.Open(confname);
    var primary, backup, testname string
    var tot int
    fmt.Fscanf(conf, "%s\n%s\n%d\n", &primary, &backup, &tot)
    cnt := tot
    ...
    for i := 0; i < tot; i++ {
        ...
        _, fail := test(primary, backup, testname)
        // res, fail := test(primary, backup, testname)
        // fmt.Printf("%s", res)
        if fail == 0 {
            fmt.Printf("\nTest case %v: success!\n\n", testname)
        } else {
    ...
    conf.Close()
    fmt.Printf("Testing done. %v out of %v passed.\n\n", cnt, tot)
    if cnt == tot {
        fmt.Printf("success\n")
    } else {
        fmt.Printf("fail\n")
